...
const EditDoctorDetails = () => {
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            //     specialization, adhar_no, father_name, mother_name, profile_image } = doctorDetails;
            const profileData = new FormData();
            for (const key in doctorDetails) {
                profileData.append(key, doctorDetails[key]);
            }
            const response = await axios.patch(`http://localhost:8000/updateuserdetails/${id}`, profileData);
    ...
    return (
        <div className='editdoctordetails'>
            <div className='editdoctorimage'><img className='editdoctorprofileimage' src={`http://localhost:8000/profileImages/`+doctorDetails.profile_image} alt='doctor'></img></div>
