...
const ViewNotificationDetail = () => {
    ...
    const getUserData = async (e) => {
        try {
            ...
            // console.log(id);
            setNotificationDetails(details.data.userData);
        } catch (error) {
            alert('error');
        }
    };
    ...
    const approveRequest = async () => {
        ...
        try {
            const response = await axios.patch(`http://localhost:8000/updateuserdetails/${id}`, notificationDetails);
            alert("approved");
    ...
    return (
        <div className='notificationdetails'>
            <div className='notificationimage'><img className='notificationprofileimage' src={`http://localhost:8000/profileImages/` + notificationDetails.profile_image} alt='notification'></img></div>
